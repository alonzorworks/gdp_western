...
st.subheader("Creating The CSV Files")
st.write("Note that this code is abridged. It does return the two dataframes used in  this project if copied and pasted into Jupyter Notebooks.")
st.code("The code works in Jupyter Labs.")
st.code("""
# Getting the Dataframe from plotly express 
import pandas as pd
...
# Getting the EU Countries from the package 
eu = EUROPEAN_UNION.names
western_countries = eu
other_countries = ["United States", "Israel", "Australia", "New Zealand", "Canada"]
western_countries.extend(other_countries)

# Limits world population to Western countries in the original DF
df2 = df[df["country"].isin(western_countries)]
df4 = df2.copy()
# Base Conversion 
# REAL GOOD CODE
# GDP Edition
# Remember the top part of range is not inclusive.
west_c = df2["country"].unique().tolist()

# Life Expectancy Edition
# Remember the top part of range is not inclusive.
life_corr = []
for c in west_c:
    for num in range(0,12):
        x = df2.loc[df2["country"] == c]["lifeExp"].iloc[0]  # Change to propper column
        y = df2.loc[df2["country"] == c]["lifeExp"].iloc[num] # Change column also
        life_corr.append((y/x) * 100)   # BASE 100 REAL Change the append

df4["life_base"] = life_corr
...
for c in west_c:
    for num in range(0,12):
        x = df2.loc[df2["country"] == c]["pop"].iloc[0]  # Change to propper column
        y = df2.loc[df2["country"] == c]["pop"].iloc[num] # Change column also
        pop_corr.append((y/x) * 100)   # BASE 100 REAL Change the append


gdp_corr = []
for c in west_c:
    for num in range(0,12):
        x = df2.loc[df2["country"] == c]["gdpPercap"].iloc[0]
        y = df2.loc[df2["country"] == c]["gdpPercap"].iloc[num]
        gdp_corr.append((y/x) * 100)   # BASE 100 REAL Change the append
    
df4["pop_base"] = pop_corr
df4["gdp_base"] = gdp_corr



# Can export two CSVs 

# df2 the base df of the west
# df4 the base 100 df of the west 
# This is done using:
# df2.to_csv("gdp_countries_western", index = False)

""")

st.write("Here is the streamlit file code.")
...
st.title("Western Countries Analysis 1952-2007")
world_map = load_lottieurl("https://assets5.lottiefiles.com/packages/lf20_b1imuadj.json")
st_lottie(world_map)
lottie_credit("World Sebastian Garcia Taborda Lottie Files")
...
def base_chart(column_name):
    fig = px.line(df_base, x = "year", y = column_name, color = "country", markers = True)
    st.plotly_chart(fig)
